...
// naive online query algorithm

public class BasicQuerySlow {
    ...
    public BasicQuerySlow(int graph[][], int vertexType[], int edgeType[], ArrayList<Integer> queryIdSet, int queryK, int schemaGraph[][]){
        this.graph = graph;
    ...
    public ArrayList<Set<Integer>> query(int l, int targetType){
            ...
            if (count == 0){
                ...
                for (int i = 0; i < metaPathListResult.size(); i++){
                    ...
                    for (int j = 0; j < metaPathListResult.get(i).size(); j++){
                        Map<Integer, int[]> pnbMap = fastBCore.queryGraphUpdate(queryIdSet, metaPathListResult.get(i).get(j), queryK);
                        graphList.add(pnbMap);
                    }
        ...
        for (int i = 0; i < finalMetaPathSet.size(); i++){
            ...
            for (int j = 0 ; j < finalMetaPathSet.get(i).size(); j++){
                Map<Integer, int[]> pnbMap = fastBCore.queryGraphUpdate(queryIdSet, finalMetaPathSet.get(i).get(j), queryK);
                graphList.add(pnbMap);
            }
            ComputeBCoreLowM computeBCoreLowM = new ComputeBCoreLowM(queryIdSet, queryK, graphList);
            Set<Integer> community = computeBCoreLowM.getBasicKPCore();
