...
// fast online query algorithm

public class BasicQueryFast {
    ...
    // for PathSim test
    private ArrayList<ArrayList<MetaPath>> resultMetaPathList = null;

    public BasicQueryFast(int graph[][], int vertexType[], int edgeType[], ArrayList<Integer> queryIdSet, int queryK, int schemaGraph[][]){
        this.graph = graph;
    ...
    public ArrayList<Set<Integer>> query(int l, int targetType){
        ...
        for (int i = 0; i < allMetaPathList.size(); i++){
            ...
            for (int j = 0; j < graph.length; j++){
                if (vertexType[j] == targetType){
                    initSet.add(j);
                }
            }

            ComputeBCoreFast computeBCoreFast = new ComputeBCoreFast(graph, vertexType, edgeType);
            Set<Integer> community = computeBCoreFast.getBasicKPCore(initSet, queryIdSet, tempMetaPathList, queryK);
            if (community != null){
        ...
        while (true){
            ...
            if (count == 0){
                ...
                for (int i = 0; i < metaPathListResult.size(); i++){
                    ...
                    ComputeBCoreFast computeBCoreFast = new ComputeBCoreFast(graph, vertexType, edgeType);
                    Set<Integer> community = computeBCoreFast.getBasicKPCore(unionSet, queryIdSet, metaPathListResult.get(i), queryK);
        ...
        if (finalMetaPathSet.size() != 0){
            ...
            resultMetaPathList = finalMetaPathSet;
        }else{
            System.out.println("No valid meta-path set!");
            resultMetaPathList = null;
        }
...
//    for PathSim test
    public ArrayList<ArrayList<MetaPath>> getResultMetaPathList(){
        return this.resultMetaPathList;
    }
}
